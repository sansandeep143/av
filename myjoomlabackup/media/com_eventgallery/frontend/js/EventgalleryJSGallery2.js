(function(Eventgallery, jQuery){
    ...
    Eventgallery.JSGallery2.prototype.createControls = function () {
        ...
        if (this.options.showCartButton) {
            this.add2cartLink = jQuery('<a href="#" id="ajax-add2cartbuttoncount" class="eventgallery-add2cart button-add2cart btn btn-primary jsgallery-add2cart"><i class="big"></i></a>');
        ...
        if (this.options.showCartConnector) {
            ...
            this.cartConnectorLink = jQuery('<a href="#" id="ajax-cartconnector" class="button-cart-connector jsgallery-cartconnector"><i class="big"></i></a>');
        ...
        if (this.options.showSocialMediaButton) {
            this.socialmediabutton = jQuery('<a id="ajax-social-media-button" class="social-share-button social-share-button-open jsgallery-socialmedia" rel="nofollow" href="#"><i class="big"></i></a>');
    ...
    */
    ...
    Eventgallery.JSGallery2.prototype.select = function (container, forceReload) {
        ...
        var url = document.location;
        var strippedUrl = url.toString().split("#");
        document.location.href = strippedUrl[0] + '#' + this.thumbs.index(container);
        ...
        if (this.options.showCartButton) {
            this.add2cartLink.data('id', source.data('id'));
            jQuery('.eventgallery-add2cart').data('id', source.data('id'));
        }

        if (this.options.showCartConnector) {
            this.cartConnectorLink.data('folder', source.data('folder'));
            this.cartConnectorLink.data('file', source.data('file'));
            this.cartConnectorLink.attr('href', decodeURIComponent(source.data('cart-connector-link')));
        }

        if (this.options.showSocialMediaButton) {
            this.socialmediabutton.data('link', decodeURIComponent(source.data('social-sharing-link')) );
        }
    ...
    Eventgallery.JSGallery2.prototype.setImage = function (newSrc, newFullSizeImage, newText, newTitle) {
        ...
        this.bigImage.parent().fadeOut(150, jQuery.proxy(function() {
            ...
            this.zoomLink.data('title', title);

            this.zoomLink.attr('href', newFullSizeImage);
			
			
			jQuery("a[data-eventgallery-lightbox='gallery']").eventgallery_colorbox({photo: true, maxWidth: '90%', maxHeight: '90%', rel: 'gallery'});
			jQuery("a[data-eventgallery-lightbox='galleryfullscreen']").eventgallery_colorbox({photo: true, maxWidth: '100%', maxHeight: '100%', rel: 'gallery'});
			if (jQuery.eventgallery_colorbox.isOpen() === true) {
