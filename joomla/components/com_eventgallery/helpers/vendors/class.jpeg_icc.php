...
class JPEG_ICC
{
    ...
    {
        if ($this->icc_profile == '') {
            throw new Exception("No profile loaded.\n");
        }
    ...
    public function SaveToICC($fname, $force_overwrite = false)
    {
        ...
        if (!$force_overwrite && file_exists($fname)) {
            throw new Exception("File $fname exists.\n");
        }

        $ret = file_put_contents($fname, $this->icc_profile);
        if ($ret === false || $ret < $this->icc_size) {
            throw new Exception ("Write failed.\n");
        }
    }
    ...
    public function RemoveFromJPEG($input, $output, $force_overwrite = false)
    {
        if (!file_exists($input)) {
            throw new Exception("File $input doesn't exist.\n");
        }
        if (!is_readable($input)) {
            throw new Exception("File $input isn't readable.\n");
        }
        $dir = realpath($output);
        if (!is_writable($dir)) {
            throw new Exception("Directory $output isn't writeable.\n");
        }
        if (!$force_overwrite && file_exists($output)) {
            throw new Exception("File $output exists.\n");
        }
    ...
    private function getJPEGSegmentContainsICC(&$f, $pos, $size)
    {
        ...
        return (bool)(substr($f, $pos + 4, self::ICC_HEADER_LEN - 2)
            == self::ICC_MARKER); // 4B offset in segment data = 2B segment marker + 2B segment size data
    }
    ...
    {
        $a = unpack(
            'Cchunk_no/Cchunk_count', substr($f, $pos + 16, 2)
        ); // 16B offset to data = 2B segment marker + 2B segment size + 'ICC_PROFILE' + 0x00, 1. byte chunk number, 2. byte chunks count
        ...
        while ($pos < $len && $counter < 100) {
            ...
            case 0xe2: // APP2
                ...
                if ($this->getJPEGSegmentContainsICC($jpeg_data, $pos, $size)) {
                    ...
                    $jpeg_data = substr_replace(
                        $jpeg_data, '', $pos, $size + 2
                    ); // remove this APP segment from dataset (segment size + 2B app marker)
                    $len -= $size + 2; // shorten the size

                    if (--$chunks_to_go == 0) {
                        return true;
                    } // no more icc profile chunks, store file
