...
class FsfModelFaq extends JModelLegacy
{
	var $_total = null;
    ...
    function &getFaq()
    {
        ...
        $query = "SELECT f.id, f.question, f.answer, f.fullanswer, f.published, f.ordering, c.title, f.faq_cat_id, f.author, f.featured FROM #__fsf_faq_faq as f LEFT JOIN #__fsf_faq_cat as c ON f.faq_cat_id = c.id";
		
		$where = array();
		$where[] = "f.id = " . FSFJ3Helper::getEscaped($db, $faqid);
		if (FSF_Helper::Is16())
		{
	...
	function getFeaturedFaqs()
	{
		...
		$qry = "SELECT * FROM #__fsf_faq_faq WHERE published = 1 AND featured = 1";
