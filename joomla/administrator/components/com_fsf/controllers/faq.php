...
class FsfsControllerFaq extends FsfsController
{
	...
	function autosort()
	{
		...
		$qry = "SELECT id FROM #__fsf_faq_faq ORDER BY question";
		$db = JFactory::getDBO();
		...
		foreach ($rows as $row)
		{
			$qry = "UPDATE #__fsf_faq_faq SET ordering = $order WHERE id = {$row->id}";
			$db->setQuery($qry);
			$db->Query(); 	
			$order++;
		}
		$this->setRedirect( 'index.php?option=com_fsf&view=faqs', "FAQs ordered alphabetically" );
	}
}
